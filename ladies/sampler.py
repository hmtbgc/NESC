...
class LadiesSampler(dgl.dataloading.BlockSampler):
    ...
    def compute_prob(self, g, seed_nodes, weight, num):
        ...
        if self.importance_sampling:
            ...
            weight = weight[out_frontier.edata[dgl.EID].long()].to(out_frontier.device)
            # f = open("LOG.txt", "w")
            # print(out_frontier.device, file=f)
            # print(weight.device, file=f)
    ...
    def generate_block(self, insg, neighbor_nodes_idx, seed_nodes, P_sg, W_sg):
        ...
        P = P_sg[u_nodes.long()].to(sg.device)
        W = W_sg[sg.edata[dgl.EID].long()].to(sg.device)
